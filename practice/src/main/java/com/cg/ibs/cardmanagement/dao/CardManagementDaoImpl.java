...
import com.cg.ibs.cardmanagement.util.DbUtil;
...
public class CardManagementDaoImpl implements CustomerDao, BankDao {
	...
	static {
		...
		CreditCardTransaction credittrans1 = new CreditCardTransaction("CRED101", "7894561239632587",
				new BigInteger("5189101213259895"), LocalDateTime.of(2018, Month.OCTOBER, 12, 11, 54, 6),
		...
		try (Connection connection = ConnectionProvider.getInstance().getConnection();
			...
			try (ResultSet resultSet = preparedStatement.executeQuery()) {
				...
				while (resultSet.next()) {
					...
					int pin=resultSet.getInt("credit_cur_pin");
					LocalDate dateOfExpiry=resultSet.getDate("credit_expiry_date").toLocalDate();
					BigInteger UCI=resultSet.getBigDecimal("UCI").toBigInteger();
					...
					crd.setCreditCurrentPin(pin);
					crd.setCreditDateOfExpiry(dateOfExpiry);
					crd.setUCI(UCI);
					crd.setCreditCardType(creditCardType);
					crd.setCreditScore(creditScore);
					crd.setCreditLimit(creditLimit);;
					crd.setIncome(income);
	...
	public void actionANDC(BigInteger debitCardNumber, Integer cvv, Integer pin, String queryId, String status) {
		BigInteger uci = new BigInteger("1223456789123456");
	...
	public void actionANCC(BigInteger creditCardNumber, int cvv, int pin, String queryId, int score, double income,
			String status) {
		BigInteger uci = new BigInteger("1231234567891456");
